...
<td>
    <input type="checkbox" class="complaint-checkbox" name="complaint_ids[]" value="{{ $complaint->id }}">
</td>
...
<form method="POST" action="{{ route('complaints.move') }}" id="moveForm">
    ...
    <div id="moveActions" class="d-none">
        <button type="button" class="btn btn-info" onclick="moveComplaints('jay')">
            Move to Jay
        </button>

        <button type="button" class="btn btn-warning" onclick="moveComplaints('others')">
            Move to Others
        </button>
    </div>
</form>
